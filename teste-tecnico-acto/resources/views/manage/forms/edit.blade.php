<x-layouts.app title="Editar Formulário">
  <div class="max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold">Editar Formulário</h2>
      <a href="{{ route('manage.forms.index') }}" class="px-4 py-2 border rounded text-sm hover:underline">Voltar</a>
    </div>

    <form method="POST" action="{{ route('manage.forms.update', $form) }}" class="space-y-6 bg-white p-6 rounded-lg shadow">
      @csrf
      @method('PUT')
      ...
      <div class="grid gap-6">
        <div>
          ...
          <input type="text" name="title" id="title" value="{{ old('title', $form->title) }}" required class="w-full border rounded px-3 py-2" />
        ...
        <div>
          ...
          <textarea name="description" id="description" rows="3" class="w-full border rounded px-3 py-2">{{ old('description', $form->description) }}</textarea>
