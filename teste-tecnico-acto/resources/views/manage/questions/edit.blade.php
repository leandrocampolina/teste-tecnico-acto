<x-layouts.app title="Editar Pergunta">
  <div class="max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold">Editar Pergunta</h2>
      <a href="{{ route('manage.questions.index') }}" class="px-4 py-2 border rounded text-sm hover:underline">Voltar</a>
    </div>

    <form method="POST" action="{{ route('manage.questions.update', $question) }}" class="space-y-6 bg-white p-6 rounded-lg shadow">
      @csrf
      @method('PUT')
      ...
      <div class="grid gap-6">
        ...
        <div>
          ...
          <textarea name="text" id="text" required rows="3" class="w-full border rounded px-3 py-2">{{ old('text', $question->text) }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input type="number" name="order" id="order" value="{{ old('order', $question->order) }}" class="w-full border rounded px-3 py-2" />
        ...
        <div class="flex items-center gap-4">
          <label class="inline-flex items-center gap-2">
            <input type="checkbox" name="is_required" value="1" {{ old('is_required', $question->is_required) ? 'checked' : '' }} class="form-checkbox" />
