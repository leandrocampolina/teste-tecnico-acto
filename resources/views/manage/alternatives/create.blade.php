<x-layouts.app title="Criar Alternativa">
  <div class="max-w-2xl mx-auto">
    <div class="flex justify-between items-center mb-6">
      <h2 class="text-2xl font-bold">Criar Alternativa</h2>
      <a href="{{ route('manage.alternatives.index') }}" class="px-4 py-2 border rounded text-sm hover:underline">Voltar</a>
    </div>

    <form method="POST" action="{{ route('manage.alternatives.store') }}" class="space-y-6 bg-white p-6 rounded-lg shadow">
      @csrf
      ...
      <div class="grid gap-6">
        ...
        <div>
          ...
          <textarea name="text" id="text" required rows="2" class="w-full border rounded px-3 py-2">{{ old('text') }}</textarea>
        ...
        <div class="grid grid-cols-2 gap-6">
          <div class="flex items-center gap-2">
            <label class="inline-flex items-center gap-2">
              <input type="checkbox" name="is_correct" value="1" old('is_correct') ? 'checked' : '' }} class="form-checkbox" />
              <span>Correta</span>
            </label>
          </div>
